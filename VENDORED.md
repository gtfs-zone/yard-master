...
| `src/modules/basemap-control.ts` | `test-track` | `src/modules/basemap-control.ts` | 868909e | modified | The basemap FAB and the appearance model, injected and persisted by the caller. See the banner's `@changes`: `ShapeToggleControl` and `MapAppearance.shapeMode` are kept, since `layer-manager.ts` here still draws a route either from its shape or stop-to-stop |
| `src/modules/layer-manager.ts` | `test-track` | `src/modules/layer-manager.ts` | 868909e | adopted | Every map layer: stops, route lines and casings, and the moving dots, fed from `GTFSScheduled`. Adopted because test-track adopted its own copy in `868909e`, rewriting it against a newly vendored `layer-specs.ts`; there is no longer a copy upstream that re-syncing this one would converge on. The banner's `@notes` record what had already diverged: a vehicle feature carries `tracker_id` beside the composite `vehicle_id`, so a click on a dot resolves to the tracker it is reporting under, and `setShapeMode` stays with `basemap-control.ts`'s toggle |
...
| `src/map-controller.ts` | `test-track` | `src/map-controller.ts` | 459c4e7 | modified | MapLibre setup, camera moves and focus. See the banner's `@changes`: the `vehicle` PageState variant became `tracker`, `VehiclePosition` carries a `trackerId`, and follow tracks a tracker rather than one of its vehicles |
