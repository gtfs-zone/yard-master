...
| `src/modules/modal-router.ts` | `test-track` | `src/modules/modal-router.ts` | ec2f5d0 | verbatim | Reconciles the modal named by the hash with the modal actually open: it opens one through a registered opener and closes one by taking `modal-utils.ts`'s stack back to the depth it had before that opener ran, so anything the modal stacked on top (a confirm, an entity form) goes with it. `ModalHost` is structural, so `createModalRouter(appState.pages)` takes this repo's `PageStateManager` unchanged. Routes `alerts` and `help`; a routed modal has to be on the modal stack, which every modal here already is. Born in coloring-book at `d8afa32`, which made the page state manager a constructor argument |
