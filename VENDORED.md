...
| `src/utils/issue-card.ts` | `test-track` | `src/utils/issue-card.ts` | 56f120a | verbatim | `renderIssueCard(title, rows)`: the warning card of label/count/note rows, empty when every count is zero. Every label and note is the caller's, which is what lets the feed page reword the unmatched-vehicle row without touching this file |
