...
from there rather than through each other. `calendar-input.ts` is the fifth, and
the only one test-track does not have at all: it edits nothing, so it has no
date to pick. `calendar-modal.ts` is the sixth, for the same reason as the
input: the month grid is coloring-book's and test-track has no calendar.
...
| `src/modules/help-modal.ts` | `test-track` | `src/modules/help-modal.ts` | 4ea08e7 | verbatim | The help viewer: maps the pages it is handed through `setHelpPages` onto sidebar entries, owns first-run state via `showHelpPageOnce`, and exports the shared `eyebrow`/`lede`/`footnote`/`glyphList` render helpers plus `installGuideButtons` |
...
| `src/modules/nav-icons.ts` | `test-track` | `src/modules/nav-icons.ts` | 615a531 | verbatim | The shared navbar/dock icon path map and `renderNavIcon`, plus the filled `renderSunIcon` / `renderMoonIcon` pair. Only `calendar`, `alerts`, `guide` and `load` are drawn here; the rest of the map is coloring-book's editor actions and is kept so the file stays one copy. Born in coloring-book; test-track carries it verbatim |
| `src/modules/navbar-actions.ts` | `test-track` | `src/modules/navbar-actions.ts` | 615a531 | verbatim | The navbar action row as a descriptor list: one `renderAction` gives every control the same box, tooltip and `aria-label`, so reordering the navbar is a data change. The list itself is the app's: this repo's is `src/modules/navbar-action-list.ts`, which is not vendored. `renderDockIcons` has no caller here, there being no mobile dock, and stays exported rather than deleted |
| `src/modules/modal-router.ts` | `test-track` | `src/modules/modal-router.ts` | 4ea08e7 | verbatim | Reconciles the modal named by the hash with the modal actually open: it opens one through a registered opener and closes one by taking `modal-utils.ts`'s stack back to the depth it had before that opener ran, so anything the modal stacked on top (a confirm, an entity form) goes with it. `ModalHost` is structural, so `createModalRouter<ModalState>(appState.pages)` takes this repo's `PageStateManager` unchanged, and the router is generic over this repo's modal union rather than importing upstream's page state types. Routes `alerts` and `help`; a routed modal has to be on the modal stack, which every modal here already is. Born in coloring-book at `d8afa32`, which made the page state manager a constructor argument |
| `src/modules/keyboard-shortcuts.ts` | `test-track` | `src/modules/keyboard-shortcuts.ts` | ec2f5d0 | verbatim | Key-string normalization, the input-field and top-modal guards, and the error toast for a handler that rejects. Holds no commands: `d8afa32` made the list a constructor argument, which is what lets this go `verbatim` instead of shipping editor shortcuts that do nothing here. This app's list is `src/modules/shortcut-list.ts`, and `describeShortcuts()` builds the guide's Keyboard Shortcuts table from the same list. `isOutsideTopModal` gates the whole dispatch, so no shortcut fires while a modal is open. Born in coloring-book |
...
| `src/modules/basemap-control.ts` | `test-track` | `src/modules/basemap-control.ts` | bac60b6 | verbatim | The basemap FAB and the appearance model, injected and persisted by the caller. Globe-only as of `bac60b6`: the projection toggle, `changeProjection` and `MapAppearance.projection` are gone from all three apps, and a `projection` stored by an earlier session is ignored on read. The shape/stops toggle is gone the same way, and a `shapeMode` stored by an earlier session is ignored too: a route draws from its shape, and stop-to-stop only when the trip has none |
...
| `src/modules/auto-zoom.ts` | `test-track` | `src/modules/auto-zoom.ts` | dc25c3a | verbatim | The auto-zoom-to-selection preference, its `localStorage` persistence, the camera gates navigation-driven moves call instead of the map, and the map-tool toggle. No app imports, MapLibre types only: the holder takes the refit callback as an argument, which here is `applyFocus` on the focus the controller last saw. Born in coloring-book at `ec21487`; taken through test-track with `map-controller.ts` |
...
| `src/modules/feed-selection.ts` | `test-track` | `src/modules/feed-selection.ts` | 4ea08e7 | verbatim | The `FeedSelection` model (`scheduled` plus `realtime`) and the error describers. Pulled in as a dependency of `feed-download.ts`; yard-master's own feed picker is not built on it, since a feed here is one of your own rows, not a URL you type. Born in coloring-book; test-track carries it verbatim |
| `src/modules/feed-url-resolve.ts` | `test-track` | `src/modules/feed-url-resolve.ts` | 4ea08e7 | verbatim | `resolveRealtimeUrl`, which takes the RT base as an argument (this app passes its own `CONFIG.RT_BASE`), plus `normalizeFeedUrl`, `validateFeedUrl`, `isLocalUrl`, `splitInnerZipPath`. Born in coloring-book; test-track carries it verbatim |
| `src/modules/about-links.ts` | `test-track` | `src/modules/about-links.ts` | 615a531 | verbatim | The About modal's shared blocks: version/source, the sibling-app links, the GTFS resources list, and the feedback list, which names the GitHub mirror alongside the Forgejo origin. Born in coloring-book; test-track carries it verbatim |
| `src/modules/help-pages.ts` | `test-track` | `src/modules/help-pages.ts` | 4ea08e7 | modified | The help page registry, and this app's help entry point in place of the old standalone `about-modal.ts`. See the banner's `@changes`: `HELP_PAGES` is `[aboutPage, shortcutsPage]` (no welcome or map-key page), the Keyboard Shortcuts page and `buildShortcutsTable` are upstream's, fed from `shortcut-list.ts` through `describeShortcuts()`, `AboutApp` is yard-master's own, which says plainly that an uploaded schedule is stored and published, and the Project and Resources blocks are rendered locally rather than through `about-links.ts`, because the shared Project block links manage.rt.gtfs.zone and this app is it |
...
| `src/types/page-state.ts` | `test-track` | `src/types/page-state.ts` | 4ea08e7 | modified | The union of every page, and the modal that can sit over one. See the banner's `@changes`: yard-master's six variants replace test-track's five, there are no list variants, and `MODAL_TYPES` is `alerts` and `help` — the calendar, sharing and the feed switcher stay unrouted |
| `src/modules/page-state-manager.ts` | `test-track` | `src/modules/page-state-manager.ts` | 4ea08e7 | modified | Owns the current focus, the navigation history and the hash. See the banner's `@changes`: the hash codec is rewritten around an explicit `type` param, and the modal dimension rides on top of it in namespaced `modal` / `modal_page` params. `clearModal` is async so the class satisfies `modal-router.ts`'s `ModalHost` unchanged |
| `src/modules/breadcrumb-trail.ts` | `coloring-book` | `src/modules/breadcrumb-trail.ts` | 3a74671 | verbatim | The crumb type vocabulary (`STOP_TYPE_LABELS`, `stopTypeLabel`), the `BreadcrumbItem<S>` shape, generic over each app's page state since `3a74671`, the two-line crumb render (`renderBreadcrumbTrail`), the header eyebrow, and `pageTitle`. Only the shell: which crumbs a page state has and how their labels are looked up stays in `breadcrumbs.ts`, since the variant set and the label sources are yard-master's own |
| `src/modules/breadcrumbs.ts` | `test-track` | `src/modules/breadcrumbs.ts` | 4ea08e7 | modified | The breadcrumb trail build and `validateState`, consuming `breadcrumb-trail.ts`'s shared shell. Every crumb label is capped at 40 characters, since a producer that writes a sentence into an alert's `header_text` would otherwise wrap a crumb over several lines. See the banner's `@changes`: the variant set is yard-master's, trackers resolve against the API list, and a managed object is accepted while its list is still empty |
| `src/modules/app-state.ts` | `test-track` | `src/modules/app-state.ts` | f1ee0ff | modified | Focus changes, modal opens and feed selection. `onStateChange` fires on every navigation and `onFocusChange` only when the page under the modal moved, so opening the guide over a page neither re-renders it nor moves the camera. See the banner's `@changes`: a feed is an API row rather than a `FeedSelection` of URLs, a linked focus is held pending until the zip it names has parsed, `loadPageData` sits on the focus side of the split, and the feed's event stream and live fleet are owned here |
| `src/types/gtfs-flex.ts` | `test-track` | `src/types/gtfs-flex.ts` | 4ea08e7 | verbatim | `StopTimeRef`, the one-of behind a stop_time's stop / location group / zone. Pulled in as `route-source.ts`'s vocabulary; this repo ingests no flex tables, so every ref it ever sees is `kind: 'stop'`. Born in coloring-book |
